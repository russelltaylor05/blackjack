...
{
   int i = 0;
   int hit = -3; /*Start at -3 to offset for known cards (0s in deck)*/
   int stand = 0;
...
/*   if (pSum > 16)
   ...
   /*TODO: Incorporate better dealer logic, this code won't execute
      ...
      if (pSum + all_cards[i] > 21)
      {
         ...
         if (all_cards[i] == 11)
         {
            ...
            if (pSum + all_cards[i] > 31)
            {
               ...
               /*printf("Player was dealt Blackjack (pSum:%i)\n",pSum);*/
               stand++;
            }
            else
            {
